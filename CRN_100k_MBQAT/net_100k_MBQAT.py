...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        e5 = self.bn5(self.conv5(e4)[:, :, :-1, :].contiguous())
        e5 = F.elu(e5, inplace=True)
...
if __name__ == '__main__':
    ...
    input = Variable(torch.FloatTensor(torch.rand(1, 16000)))
    inp_mag, inp_pha = torch_stft.stft(input)

    macs, params = profile(net, inputs=(inp_mag,))

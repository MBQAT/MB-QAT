...
from Checkpoint_StQAT import CheckpointSt
from standard_QAT import prepare
from net_100k_StQAT import Net
...
class Model(object):
    ...
    def train(self, args):
        ...
        qat_config = {'quant_bit': 8, "all_positive": False, "per_channel": False, "batch_init": 20}
        prepare(network, inplace=True, quant_bits=qat_config["quant_bit"],
        ...
        for epoch in range(start_epoch, args.max_epoch):
            ...
            for i, (mixtures, labels, noises) in enumerate(tr_batch_dataloader.dataloader):
                ...
                if (i + 1) % args.eval_steps == 0:
                    ...
                    qat_config2 = {'quant_bit': 8, "all_positive": False, "per_channel": False, "batch_init": 20}
                    prepare(model_copy, inplace=True, quant_bits=qat_config2["quant_bit"],
                    ...
                    checkpoint = CheckpointSt(epoch + 1, avg_train_loss, best_loss, network.module.state_dict(),
                    ...
                    network.train()
                    cnt = 0.
            shutil.copy('train_100k_StQAT.log', args.log_path + 'train_100k_StQAT.log')
            scheduler.step()
    ...
    def test(self, args):
        ...
        qat_config = {'quant_bit': 8, "all_positive": False, "per_channel": False, "batch_init": 20}
        prepare(net, inplace=True, quant_bits=qat_config["quant_bit"],
                all_positive=qat_config["all_positive"], per_channel=qat_config["per_channel"],
                batch_init=qat_config["batch_init"])
...
if __name__ == '__main__':
    ...
    with open('config_100k_StQAT.yaml', 'r') as f_yaml:

...
from Checkpoint_tcn_MBQAT import CheckpointTCN
from learn_scale_tcn_MBQAT import prepare
from net_tcn_MBQAT import TCN
...
class Model(object):
    ...
    def train(self, args):
        ...
        network = TCN(input_size=161, output_size=161, num_channels=[40, 80], kernel_size=3, dropout=0.3)
        ...
        for epoch in range(start_epoch, args.max_epoch):
            ...
            for i, (mixtures, labels, noises) in enumerate(tr_batch_dataloader.dataloader):
                ...
                if (i + 1) % args.eval_steps == 0:
                    ...
                    model_copy = TCN(input_size=161, output_size=161, num_channels=[40, 80], kernel_size=3, dropout=0.3)

                    qat_config2 = {'a_bit': 8, 'w_bit': 8, "all_positive": False, "per_channel": False,
                                   "batch_init": 20}
                    prepare(model_copy, inplace=True, a_bits=qat_config2["a_bit"], w_bits=qat_config2["w_bit"],
                            all_positive=qat_config2["all_positive"], per_channel=qat_config2["per_channel"],
                            batch_init=qat_config2["batch_init"])

                    model_copy.cuda()
                    model_copy.load_state_dict(state_dict)

                    avg_eval_loss = self.validate(model_copy, cv_batch_dataloader, torch_stft, criterion)
                    ...
                    checkpoint = CheckpointTCN(epoch + 1, avg_train_loss, best_loss, network.module.state_dict(),
                                               optimizer.state_dict())
                    model_name = args.model_path + '{}-{}-val.ckpt'.format(epoch + 1, i + 7)
                    ...
                    network.train()
                    cnt = 0.
            shutil.copy('train_tcn_MBQAT.log', args.log_path + 'train_tcn_MBQAT.log')
            scheduler.step()
    ...
    def test(self, args):
        ...
        net = TCN(input_size=161, output_size=161, num_channels=[40, 80], kernel_size=3, dropout=0.3)
...
if __name__ == '__main__':
    ...
    with open('config_tcn_MBQAT.yaml', 'r') as f_yaml:

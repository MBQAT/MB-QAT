...
def quantize_model(model, mode, data_args=None):
    ...
    if mode == 'static':
        ...
        quan_checkpoint = Checkpoint(checkpoint.start_epoch, checkpoint.train_loss, best_loss,
...
if __name__ == '__main__':
    ...
    if args.PTQ_type == 'd':
        model_q = quantize_model(model_to_quantize, 'dynamic')
    else:
        model_q = quantize_model(model_to_quantize, 'static', args)

    rod_input = Variable(torch.FloatTensor(torch.rand(1, 99, 161)))
    macs, params = profile(model_q, inputs=(rod_input,))
